                ...
                final Thread thread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            tele = findViewById(R.id.UserPhone);
                            pwd = findViewById(R.id.UserPassword);

                            String uname = name.getText().toString().trim();
                            String uphone = tele.getText().toString().trim();
                            String upwd = pwd.getText().toString().trim();

                            String sql = "select username from user_info where username = 'uname'";
                            Statement st = (Statement)conn.createStatement();
                            ResultSet rs = st.executeQuery(sql);

                            //若无重复用户名则注册成功，否则失败
                            if(rs == null){
                                String sql1 = "insert into user_info (username, password, telephone, expenditure) values (uname, upwd, uphone, 0);";
                                ...
                                //跳转到主页
                                Intent intent = new Intent(ResignActivity.this, HomepageActivity.class);
