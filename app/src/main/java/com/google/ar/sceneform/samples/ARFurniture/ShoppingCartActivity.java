...
public class ShoppingCartActivity extends AppCompatActivity implements GoodsCallback, View.OnClickListener {
     ...
     * 在其他页面，添加商品到购物车
     */
    public void addToCart(CarResponse.OrderDataBean.CartlistBean info) {
        if(info==null) {
            return;
        }
        String proName = info.getProductName();
        ...
        //没有商店
        CarResponse.OrderDataBean mShop = new CarResponse.OrderDataBean();
        mShop.setShopName(shopName);
        mShop.getCartlist().add(info);
        mList.add(mShop);
        shopIdList.add(mList.size()-1);
    }
